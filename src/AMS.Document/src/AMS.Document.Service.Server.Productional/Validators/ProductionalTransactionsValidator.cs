...
namespace Undersoft.AMS.Document.Service.Server.Productional.Validators
{
    ...
    public class ProductionalTransactionsValidator : CommandSetValidator<ProductionalTransaction>
    {
        public ProductionalTransactionsValidator(IServicer servicer) : base(servicer)
        {
            ValidationScope(
                OperationType.Any,
                () =>
