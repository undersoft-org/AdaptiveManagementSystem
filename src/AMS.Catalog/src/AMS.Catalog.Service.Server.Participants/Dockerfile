...
WORKDIR /src
COPY ["AMS/src/AMS/src/AMS.Catalog/src/AMS.Catalog.Service.Server.Participants/Undersoft.AMS.Catalog.Service.Server.Participants.csproj", "AMS/src/AMS/src/AMS.Catalog/src/AMS.Catalog.Service.Server.Participants/"]
...
RUN dotnet restore ./AMS/src/AMS/src/AMS.Catalog/src/AMS.Catalog.Service.Server.Participants/Undersoft.AMS.Catalog.Service.Server.Participants.csproj
COPY . .
WORKDIR /src/AMS/src/AMS/src/AMS.Catalog/src/AMS.Catalog.Service.Server.Participants
RUN dotnet build ./Undersoft.AMS.Catalog.Service.Server.Participants.csproj -c $BUILD_CONFIGURATION -o /app/build

# This stage is used to publish the service project to be copied to the final stage
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish ./Undersoft.AMS.Catalog.Service.Server.Participants.csproj -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false

# This stage is used in production or when running from VS in regular mode (Default when not using the Debug configuration)
FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "Undersoft.AMS.Catalog.Service.Server.Participants.dll"]